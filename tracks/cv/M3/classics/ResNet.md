# ResNet（2015）：深度革命的残差解法

> 网络加深到一定程度反而变差？ResNet 用一条"跳跃连接"让梯度直通底层，把网络做到上百层，拿下当年所有视觉赛事。残差连接后来出现在 Transformer 等几乎所有现代架构里——这是本系列最重要的一篇。

**前置知识**：[VGGNet](VGGNet.md)、[M2 反向传播](../../M2/L02-反向传播.md)（理解梯度消失）。
**预计投入**：阅读 1.5-2 小时 + 动手 1 小时。
