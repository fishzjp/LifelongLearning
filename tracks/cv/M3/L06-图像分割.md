...
> 检测画框，分割描边：给每个像素贴上类别标签。医疗影像勾病灶、自动驾驶分车道、抠图换背景，都是分割。本课从 FCN 讲到 U-Net（医疗标配）、DeepLab、Mask R-CNN，并说清 Dice、mIoU 这些指标怎么算。

**前置知识**：[L05-目标检测](L05-目标检测.md)（Mask R-CNN 建立在检测之上）；熟悉 PyTorch。
...
![U-Net编码器-解码器架构图](../assets/Gemini_Generated_Image_c3iy6gc3iy6gc3iy.png "U-Net编码器-解码器架构图")
