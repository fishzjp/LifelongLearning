...
> 分类回答"是什么"，检测还要回答"在哪里"：框出每个物体并命名。本课讲两条技术路线——两阶段（R-CNN 系：先找候选再分类，准但慢）与单阶段（YOLO/SSD：一步到位，快）——以及 IoU、mAP、NMS 这些绕不开的概念。

**前置知识**：[L01-卷积](L01-卷积.md) 与经典 backbone（[classics/](classics/README.md) 至少读完 ResNet）。
**预计投入**：每天 1-2 小时，约 1-2 周。

![R-CNN vs YOLO vs SSD 目标检测方法对比图](../assets/Gemini_Generated_Image_bhkkxabhkkxabhkk.png "R-CNN vs YOLO vs SSD 目标检测方法对比图")
