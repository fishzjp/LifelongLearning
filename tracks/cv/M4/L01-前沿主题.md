...
> 前面里程碑的"现在"，这一课是"趋势"：Transformer 全面进军视觉（ViT/Swin/DETR）、不用标签的自监督学习（SimCLR/MAE）、图文对齐的多模态（CLIP）、生成模型（GAN/Diffusion）、以及把模型塞进生产的压缩部署。按兴趣选读小节。

**前置知识**：[M3](../M3/README.md)（尤其 [L03-注意力机制](../M3/L03-注意力机制.md)）；其余按小节要求。
**用法**：地图式浏览 + 按方向精读，不必按顺序。

---

## 章节概览
本课涵盖当前计算机视觉领域最热门和最具前景的研究方向：
