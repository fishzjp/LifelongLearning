...
> 轨道的毕业考场：四个进阶项目（人脸识别、工业检测、医学分割、视频分析）在本文档内逐个展开，含完整代码与部署方案。在此之前的两个入门项目分别属于 [M0](../M0/README.md) 与 [M3](../M3/README.md)，见下表回链。

**前置知识**：[M3 过关](../M3/README.md)；各项目另有所标注的专项要求。
**预计投入**：每个项目 1-5 天不等，按表格中的预估。
...
| 项目 | 所属 | 核心技术 | 预计时间 |
|------|------|----------|----------|
| [图像分类：MNIST 手写数字](../M0/PROJECT-MNIST.md) | M0 | LeNet + PyTorch 完整流程 | 1-2 天 |
| [目标检测：YOLOv5 口罩检测](../M3/PROJECT-YOLO.md) | M3 | 自定义数据集训练 | 3-4 天 |

### Capstone 项目（本文档内，四选一过关）

| 项目 | 核心技术 | 预计时间 | 应用场景 |
|------|----------|----------|----------|
| 项目1：人脸检测与识别系统 | MTCNN + FaceNet | 2-3 天 | 智能门禁、考勤 |
| 项目2：智能目标检测器 | YOLOv8 + 数据标注 | 3-5 天 | 安防、自动驾驶 |
| 项目3：医学图像分割 | U-Net + 医学影像 | 3-4 天 | 病灶检测、辅助诊断 |
| 项目4：视频行为分析 | 目标跟踪 + 动作识别 | 5-7 天 | 智能监控 |
