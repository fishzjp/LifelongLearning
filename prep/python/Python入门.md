...
> 从变量到 OpenCV，一篇补齐写 CV 代码需要的全部 Python 技能。学完你能：用 NumPy 高效操作数组（图像的载体）、用 Matplotlib 可视化结果、用 OpenCV 读写和处理图像。

**前置知识**：无，从零教起（已有 Python 经验的可跳到"NumPy 数组操作"一节）。
**预计投入**：每天 1-2 小时，约一周。每个代码块都建议亲手运行。
